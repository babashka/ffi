...
;; The generated trampolines ns exists only in babashka's own image. In a
;; third-party image the FFM handles serve every shape, so a missing ns
;; means no trampolines, not an error.
(def ^:private trampoline-ids
  (when native-image?
    (try @(requiring-resolve 'babashka.impl.ffi-trampolines/ids)
         (catch java.io.FileNotFoundException _ nil))))

(def ^:private trampoline-invoker
  (when native-image?
    (try (requiring-resolve 'babashka.impl.ffi-trampolines/invoker)
         (catch java.io.FileNotFoundException _ nil))))
