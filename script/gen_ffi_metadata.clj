...
;; resources-native/META-INF/native-image/babashka/ffi/reachability-metadata.json
;;
...
;; - src-native/babashka/ffi/impl/ffi_trampolines.clj: shape key -> builder fn map used
...
;; stack does: macOS on AArch64 packs a stack slot to the width of the
;; argument, where every other ABI here gives it eight bytes, so a long in
...
(def max-int-args
  (if (and (= "aarch64" (System/getProperty "os.arch"))
...
(def ordered-shapes
  ...
  (concat
   (for [n (range 0 7)
         ...
         :when (fp-ok? args)]
     (vec args))
   (map #(vec (repeat % "jlong")) (range 7 (inc max-int-args)))))
...
;; the generated artifacts sit outside :paths, so babashka's own build does
;; not pick them up: it still generates and uses its own until it opts in
(doseq [d ["resources-native/META-INF/native-image/babashka/ffi"
           "src-java/babashka/ffi/impl"
           "src-native/babashka/ffi/impl"]]
  (.mkdirs (java.io.File. d)))

(spit "resources-native/META-INF/native-image/babashka/ffi/reachability-metadata.json"
...
(def sorted-shapes
  ...
  (concat
   ...
   ;; sort is the identity: EVP_PBE_scrypt takes ten. max-int-args is where
   ;; the arguments start reaching the stack
   (for [a (range (inc MAX-ARITY) (inc max-int-args))]
     (shape a 0 0))))
...
(spit "src-native/babashka/ffi/impl/ffi_trampolines.clj"
      (str ";; Generated by script/gen_ffi_metadata.clj. Do not edit.\n"
